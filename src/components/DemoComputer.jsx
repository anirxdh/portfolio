...
const DemoComputer = ({ textures: textureSources, activeIndex = 0, ...props }) => {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF('/models/computer.glb');
  const { actions } = useAnimations(animations, group);

  const sources = textureSources && textureSources.length ? textureSources : ['/textures/project/lark-1.jpg'];
  const key = sources.join(',');
  const maps = useTexture(sources);
  const textures = Array.isArray(maps) ? maps : [maps];

  const matRef = useRef();
  const stateRef = useRef({ key: null, idx: 0 });
  const [displayIdx, setDisplayIdx] = useState(0);

  // Keep texture orientation/color correct whenever the images (re)load.
  useEffect(() => {
    textures.forEach((t) => {
      t.flipY = false;
  ...
  // Snap to the first image on project change; crossfade (fade-through) on image change.
  // stateRef tracks the shown index reliably, independent of async setState timing.
  useEffect(() => {
    ...
    const target = textures[activeIndex] ? activeIndex : 0;
    if (target === st.idx) return undefined;
    st.idx = target;
    ...
    <group ref={group} {...props} dispose={null}>
      <group name="Scene">
        <mesh
          ...
          <meshBasicMaterial ref={matRef} map={textures[displayIdx]} transparent toneMapped={false} />
        </mesh>
